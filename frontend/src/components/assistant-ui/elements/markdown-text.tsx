...
const MarkdownTextImpl: FC<MarkdownTextProps> = ({ components }) => {
  ...
  const markdownComponents = useMemo(() => {
    if (!stableComponents) return defaultMarkdownComponents;
    return {
...
// 供自定义渲染（如知识问答的引用角标）复用默认 markdown 组件集
export const defaultMarkdownComponents = memoizeMarkdownComponents({
  h1: ({ className, ...props }) => (
    <h1
      className={cn(
