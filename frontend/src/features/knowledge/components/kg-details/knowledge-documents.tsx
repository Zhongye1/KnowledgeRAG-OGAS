...
import { buildDocumentPollSignature, useDocumentPolling } from '../../hooks/use-document-polling'
...
export function KnowledgeDocuments({ kb, onBack }: KnowledgeDocumentsProps) {
  ...
  const totalPages = Math.max(
    1,
    documentsQuery.data?.total_pages ?? Math.ceil(total / 20),
  )
    ...
    <Card size="sm" data-slot="knowledge-documents-panel">
      <CardHeader className="flex-row items-center justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          >
            <ArrowsClockwise
              className={cn('size-4', documentsQuery.isFetching && 'animate-spin')}
            />
          </Button>
      ...
      <Dialog open={batchDeleteOpen} onOpenChange={setBatchDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              确定删除选中的 {selectedIds.size} 个文档？将同时清理对象存储文件与关联登记。
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <DialogClose asChild>
