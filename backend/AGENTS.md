...
**检索/问答**：检索 canonical 端点 `POST /api/v1/rag/search`（扁平跨库 `kb_names` + `document_ids` 直推，响应 `sources{text,image}` + `route/steps`，EagleRAG 对齐）与 `/rag/search/stream`（SSE）；编排已事件化为 `RetrievalService._asteps`（步骤即时产出，同步门面经 `_collect` 消费）；`GET /rag/images/{image_id}/url` 视觉 tile 预签名回源。问答 `app/chat/api/v1/chat.py`（EventSourceResponse）→ `ChatService.astream` → 同一五步编排：① KB 归属/ACL 校验（scope 表达式由 `retrieval/service/scope.py` 生成 Milvus 标量过滤）② embedding（model_provider）③ 策略召回（`RETRIEVE_STRATEGIES` 注册表 → `milvus_kb_ops.search_ragf_kb`，dense+sparse RRF；可选视觉召回 `strategies/visual.py` → `milvus_visual_ops.search_visual`，命中独立 `visual_results` 返回，失败降级）④ reranker 精排（失败降级为召回序）⑤ 回查 PG chunks 补全来源 → `build_citations` → chat model 流式输出 `delta` 帧。
