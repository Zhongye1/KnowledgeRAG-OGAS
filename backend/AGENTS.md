...
| 文档解析 | rapidocr / pypdfium2 / python-docx / python-pptx / MinerU 公有云 processor | 按文件类型由解析工厂路由                                                                                                                                       |
...
**摄取（Celery 异步，双管线 spec：docs/specs/2026-09-10-dual-pipeline-ingest-design.md）**：上传/URL 端点 `send_task` 入队（ingest/knowhere/visual 三队列可独立路由）→ `ingest/tasks/tasks.py: ingest.process_document` 为**路由入口**：策略链（`ingest/routing/`，默认 auto：knowhere 扩展集含 pdf/docx/pptx/md/txt/csv，图片/扫描件走 visual）决定管线。legacy = 既有工厂链（引擎不可用时的兜底）；knowhere = Knowhere SDK 语义解析（`ingest/service/knowhere_service.py`）；visual = PixelRAG 视觉切片（`ingest/service/visual_service.py`）。三条管线共享：**先 Milvus 后 PG 双写**（失败删向量补偿）、`ingest_jobs` 细粒度审计（`ingest/model/ingest_job.py`，job_id = Celery task_id）、dedup 成功后置登记（D9）、限额三道关（`ingest/limits.py`）；`ingest.reconcile` 由 beat 周期对账（含视觉管线），PG 为事实源。引擎依赖（knowhere/pixelrag）为可选组件、惰性导入，不可用管线路由期显式回退 legacy。
