...
**摄取（Celery 异步）**：上传端点 `send_task` 入队（`RAGF_CELERY_INGEST_QUEUE` 可独立路由）→ `ingest/tasks/tasks.py: ingest.process_document`：状态认领 → 解析工厂路由引擎 → MinIO 落盘 → 分块 → embedding → **先 Milvus 后 PG 双写** → 状态回写；`ingest.reconcile` 由 beat 周期对账，PG 为事实源。
