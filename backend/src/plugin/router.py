...
from backend.src.plugin.status import PluginStatusChecker
from backend.src.utils.dynamic_import import import_module_cached
...
def inject_extend_router(plugin: PluginEntry) -> None:
    ...
    if not os.path.exists(plugin_api_path):
        raise PluginConfigError(
            f"插件 {plugin.name} 缺少 api 目录，请检查插件文件是否完整"
        )
    ...
    for root, _, api_files in os.walk(plugin_api_path):
        for file in api_files:
            ...
            path_to_module_str = os.path.relpath(file_path, PLUGIN_DIR).replace(
                os.sep, "."
            )[:-3]
            module_path = f"backend.src.plugin.{path_to_module_str}"
            ...
            try:
                ...
                target_module_path = (
                    f"backend.src.app.{app_name}.api.{relative_path.replace(os.sep, '.')}"
                )
                ...
                if not target_router or not isinstance(target_router, APIRouter):
                    raise PluginInjectError(
                        _invalid_router_msg(plugin.name, module_path, "扩展级")
                    )
                ...
                raise PluginInjectError(
                    f"扩展级插件 {plugin.name} 路由注入失败：{e!s}"
                ) from e
...
def inject_app_router(plugin: PluginEntry, target_router: APIRouter) -> None:
    ...
    :return:
    """
    module_path = f"backend.src.plugin.{plugin.name}.api.router"
    try:
        module = import_module_cached(module_path)
        routers = plugin.routers
        if not routers or not isinstance(routers, list):
            raise PluginConfigError(
                f"应用级插件 {plugin.name} 配置文件存在错误，请检查"
            )
        ...
        for router in routers:
            ...
            if not plugin_router or not isinstance(plugin_router, APIRouter):
                raise PluginInjectError(
                    _invalid_router_msg(plugin.name, module_path, "应用级")
                )

            # 将插件路由注入到目标路由中
            target_router.include_router(
                plugin_router, dependencies=[Depends(PluginStatusChecker(plugin.name))]
            )
    except Exception as e:
...
def build_final_router() -> APIRouter:
    """构建最终路由"""
    from backend.src.plugin.core import parse_plugin_config, resolve_plugin_order
    ...
    from backend.src.app.router import router as main_router
