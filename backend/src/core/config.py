...
class Settings(BaseSettings):
    ...
    TOKEN_REQUEST_PATH_EXCLUDE_PATTERN: list[
        Pattern[str]
    ] = []  # JWT / RBAC 路由白名单（正则）
    ...
    DATA_PERMISSION_MODEL_TEMPLATE_VARIABLES: list[
        dict[str, str]
    ] = [  # 数据规则模型可用模板变量
        {"key": "__ALL__", "comment": "所有模型"},
    ]
    DATA_PERMISSION_COLUMN_TEMPLATE_VARIABLES: list[
        dict[str, str]
    ] = [  # 数据规则字段可用模板变量
        {"key": "__dept_id__", "comment": "部门 ID"},
        {"key": "__created_by__", "comment": "创建者"},
    ]
    DATA_PERMISSION_TEMPLATE_VARIABLES: list[
        dict[str, str]
    ] = [  # 数据规则值可用模板变量
