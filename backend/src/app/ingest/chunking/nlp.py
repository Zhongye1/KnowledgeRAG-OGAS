...
def is_english(texts: str | list[str]) -> bool:
    ...
    seq = [texts] if isinstance(texts, str) else [t for t in texts if isinstance(t, str) and t.strip()]
...
def is_probable_heading_line(line: str) -> bool:
    ...
    return not (text.endswith(('。', '；', '！', '!', '？', '?')) and len(text) > 20)
...
def bullets_category(sections: list[str]) -> int:  # ruff:ignore[complex-structure]
...
def remove_contents_table(sections: list[str] | list[tuple[str, str]], *, eng: bool = False) -> None:  # ruff:ignore[complex-structure]
    i = 0
...
def tree_merge(bull: int, sections: list[str] | list[tuple[str, str]], depth: int) -> list[str]:  # ruff:ignore[complex-structure]
...
def hierarchical_merge(bull: int, sections: list[str] | list[tuple[str, str]], depth: int) -> list[list[str]]:  # ruff:ignore[complex-structure]
    if not sections or bull < 0:
        return []
...
def naive_merge(  # ruff:ignore[complex-structure]
    ...
    def add_chunk(text: str, pos: str) -> None:
        ...
        if not chunks[-1] or token_nums[-1] > threshold:
            if chunks:
