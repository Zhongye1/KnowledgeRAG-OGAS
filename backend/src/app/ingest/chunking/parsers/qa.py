...
def _extract_pairs_from_markdown_headings(markdown_content: str) -> list[tuple[str, str]]:  # ruff:ignore[complex-structure]
...
def chunk_markdown(filename: str, markdown_content: str, parser_config: dict[str, Any] | None = None) -> list[str]:  # ruff:ignore[complex-structure]
    ...
    if suffix in {'.xlsx', '.xls'}:
        ...
        delimiter = _guess_delimiter(lines)
        pairs.extend(_extract_pairs_with_delimiter(lines, delimiter))
    elif suffix in {'.md', '.markdown', '.mdx'} or suffix == '.docx':
