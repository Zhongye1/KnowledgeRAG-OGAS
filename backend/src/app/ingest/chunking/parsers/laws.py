...
def _ensure_chunk_token_limit(  # ruff:ignore[complex-structure]
    ...
    for chunk in normalized:
        ...
        refined = nlp.naive_merge(
            [(line, '') for line in _iter_lines(chunk)],
        ...
        for item in refined:
            ...
            if nlp.count_tokens(cleaned) <= max_tokens:
                ...
                sentence_refined = nlp.naive_merge(
                    [(sentence, '') for sentence in re.split(r'(?<=[。！？；;!?])', cleaned) if sentence.strip()],
                    chunk_token_num=max_tokens,
                    delimiter=delimiter,
                    overlapped_percent=overlapped_percent,
                )
