...
from backend.src.common.model import Base, id_key
...
class Dept(Base):
    ...
    leader: Mapped[str | None] = mapped_column(
        sa.String(32), default=None, comment="负责人"
    )
    phone: Mapped[str | None] = mapped_column(
        sa.String(11), default=None, comment="手机"
    )
    email: Mapped[str | None] = mapped_column(
        sa.String(64), default=None, comment="邮箱"
    )
    status: Mapped[int] = mapped_column(default=1, comment="部门状态(0停用 1正常)")

    # 父级部门
    parent_id: Mapped[int | None] = mapped_column(
        sa.BigInteger, default=None, index=True, comment="父部门ID"
    )
