...
from backend.src.common.dataclasses import (
    AccessToken,
    NewToken,
    RefreshToken,
    TokenPayload,
...
async def create_access_token(
    user_id: int, *, multi_login: bool, **kwargs
) -> AccessToken:
    """
    生成加密 token
...
async def create_refresh_token(
    session_uuid: str, user_id: int, *, multi_login: bool
) -> RefreshToken:
    """
...
async def create_new_token(
    ...
    **kwargs,
    ...
    new_access_token = await create_access_token(
        user_id, multi_login=multi_login, **kwargs
    )
...
async def jwt_authentication(token: str) -> GetUserInfoWithRelationDetail:
    ...
    redis_token = await redis_client.get(
        f"{settings.TOKEN_REDIS_PREFIX}:{ctx.user_id}:{token_payload.session_uuid}"
    )
    if not redis_token:
        raise errors.TokenError(msg="Token 已过期")
