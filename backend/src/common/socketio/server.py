...
async def connect(sid, environ, auth) -> bool:
    ...
    if token == settings.WS_NO_AUTH_MARKER:
        ...
        await redis_client.set(
            f"{settings.TOKEN_ONLINE_REDIS_PREFIX}:sid:{sid}", session_uuid
        )
        await redis_client.sadd(
            f"{settings.TOKEN_ONLINE_REDIS_PREFIX}:session:{session_uuid}", sid
        )
    ...
    try:
        with request_cycle_context(
            {settings.TRACE_ID_REQUEST_HEADER_KEY: uuid.uuid4().hex}
        ):
...
async def disconnect(sid) -> None:
    """Socket 断开连接事件"""
    session_uuid = await redis_client.get(
        f"{settings.TOKEN_ONLINE_REDIS_PREFIX}:sid:{sid}"
    )
    if not session_uuid:
        return
